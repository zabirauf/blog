...
I used [Kobo Nia](https://us.kobobooks.com/products/kobo-nia) for this as its cost was $100 and its a newer e-device which means it will have better processor, good resolution. Though Kobo Nia is not officially supported by the XCSoar but unofficially it works with certain version.

I got the [BlueFly TTL GPS](https://www.blueflyvario.com/product/blueflyvario_ttl_gps_v12-2/) version which works with the Kobo Nia and it cost was approx. $75 USD ($105 AUD as its shipped from Australia).

The BlueFly needs to be soldered to Kobo so you need a soldering iron and some soldering skills.
I also desgined a 3D printed case to hold both of them together so if you have 3D printer laying around you can also print that. The case isn't yet designed to be mounted, I'm working on a solution for that. Alternatively you can always hotglue BlueFly directly to you Kobo.

## 1. Backup Kobo and install XCSoar

Kobo Nia comes with an 8GB <a href="/static/images/blog/glider_flight_computer/sd-card-sizes.jpg" target="_blank">MicroSD memory card</a> but it's not directly accessible. The reason you should backup the SD card is because you will be installing XCSoar and if something goes wrong, you can always flash the SD card to its factory state and recover. I made the mistake of not doing it and I ran into a problem, which gladly I was able to get out of it, but momentary I was in a panick thinking my newly bought $100 Kobo may be a paperweight.
...
Once you have the case open, disconnect the powercable so you don't short anything. After that you can pull the SD card out. The SD card has some glue so you may need to shimmy it to loosen up and then take out.
...
The final step is to eject the SD card and put the SD card back, plugin in battery cable back and power it up. This will install XCSoar and when you poweron it will land you on the following screen.
...
You can use the **Nickel** to go back to the original Kobo Nia e-book software. That would also be handy when next time you have to access the SD card without opening the backcover, for putting maps, waypoints and other files for XCSoar.

## 2. Soldering BlueFly Vario

Make sure the battery cable is popped of before you do anything.
If this is first time you are soldering, get some practice on some wires to a broken circuit. The BlueFly Vario has the following layout
...
The BlueFly Vario has the Serial Input/Output connectors which will connect to Kobo Nia connectors. When you open Kobo Nia these connectors will be on the back right side. The layout of the connection should be following. Make sure that **Rx** from one device goes to **Tx** on the other device and vice versa. **Rx** is receiver and **Tx** transmission, hence transmitted data from one device will go to receiveing end of the other. For the Ground wire from BlueFly vario, instead of connecting directly to Kobo, I connected that to one of the screw which is also ground.
...
Once you have the BlueFly connected, you can start setting up the XCSoar so you can use BlueFly, download maps etc. Once you bootup, go to **Fly** mode.
...
1. You can start with the config file <a href="/static/documents/BlueFly Menu.xci" target="_blank">here by downloading it</a>
...
The map you generate shouldn't be very large (i.e. not whole of USA) as that will make loading XCSoar so slow that it may not be practically usable. Generally keep it to the state or region you are flying. You can generate multiple maps, put all of the on device and then select later which you want to use.
...
When soaring along with airspace information you may also need latest TFRs so you don't violate them. You can use [soaringdata.info](http://soaringdata.info) to generate various airspace data and use them XCSoar, including TFRs. Where there is option of file type use the **OpenAir (.TXT)**.
